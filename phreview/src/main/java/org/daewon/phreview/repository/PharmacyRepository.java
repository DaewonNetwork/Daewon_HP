...
//    List<Pharmacy> findByLoc(double lat, double lng);
    ...
    @Query(value = "SELECT ph_id,enjoy_index,ph_add,ph_name,ph_tel,phx,phy,star_index,time_holi_end_date,time_holi_start_date,time_sat_end_date ,time_sat_start_date,time_week_end_date,time_week_start_date, CEIL((SELECT COUNT(*) FROM pharmacy p2 WHERE p2.ph_id <= p1.ph_id AND (6371 * acos(cos(radians(:lat)) * cos(radians(phY)) * cos(radians(phX) - radians(:lng)) + sin(radians(:lat)) * sin(radians(phY)))<=1))/10.0) as ph_page_index, CEIL((SELECT COUNT(*) FROM pharmacy p2 WHERE (6371 * acos(cos(radians(:lat)) * cos(radians(phY)) * cos(radians(phX) - radians(:lng)) + sin(radians(:lat)) * sin(radians(phY)))<=1))/10) as ph_page_total, (6371*acos(cos(radians(:lat))*cos(radians(phY))*cos(radians(phX)-radians(:lng))+sin(radians(:lat))*sin(radians(phY)))) AS distance FROM pharmacy p1 HAVING distance <= 1",nativeQuery = true)
    List<Pharmacy> findByLoc(double lat, double lng);
